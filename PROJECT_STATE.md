...
- **TASK-FE-INTERVIEWER-001**：**Open（2026-08-14 已实现，commit `233828c`；待 WSL 验证）**——Interviewer 个人中心：DashboardView（默认页，统计卡+未来 7 天即将面试+快捷操作，未登录引导卡）+ 历史会话真实化（登录拉 `/conversations`，点击会话恢复至 ChatPanel（拉 messages + 发送带 conversation_id 持久化），新建对话清空回匿名）+ 管理入口按 role 隐藏（仅 owner_admin）。已知限制：InterviewView 页内登录后 user state 不即时回填（刷新同步）；会话无重命名/删除。前端门禁待用户 WSL（沙箱缺 win32 平台包）。详见任务单 `tasks/TASK-FE-INTERVIEWER-001.md`。
- **TASK-KB-RAG-001**：**Open（2026-08-14 已实现并验证，commit `25a3fc3`，verified；待用户授权关闭）**——RAG 检索质量工程：迁移 0006 `knowledge_chunks`（chunk 级 embedding，0005 doc.embedding 列弃用）+ `chunking.py`（500/50 切分边界优先）+ `bm25.py`（纯 Python Okapi + RRF 融合）+ 混合检索（向量 top10 + BM25 top10 → RRF top6 → citations `doc·chunk`）；契约不变。**用户 WSL 验证 12 passed in 13.67s**（迁移 5 + 知识库 7，含长文档中段埋词 chunk 级命中），`verified_commit=25a3fc3`。详见任务单 `tasks/TASK-KB-RAG-001.md`。
- **TASK-DEPLOY-001**：**Open（2026-08-13 素材已落地，commit `f12e9c9` + `fef6b26`；待用户配 DeepSeek key）**——简历素材（真版 PDF）+ DeepSeek V4 Flash 支持（embedding 配置拆分，DeepSeek 无 /embeddings 端点）。API key 由用户运行时环境变量提供（不落文件）。详见任务单 `tasks/TASK-DEPLOY-001.md`。
