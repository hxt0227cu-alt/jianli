...
- **TASK-M1-APPOINTMENTS**：**Open（实现已交付，待本机验证批处理）**——合并同域主线：我的预约列表 + 改期（原子换格/会议号原地改）+ 取消；后端三接口 `listMyAppointments`/`updateAppointment`/`cancelAppointment` + 真实 PG/Redis 测试 `test_management.py` + 前端 `my-appointments.tsx`；复用现有列、无新迁移、不单列 REVIEW 任务。提交：`6483ba0`（主体）+ `7da4fad`（list_my 收窄为 active 修复）。**验证批处理（pytest+ruff+mypy+pnpm typecheck/build/test）需在用户本机真实环境运行——sandbox 无 Docker/PG/Redis/venv/node_modules，仅 py_compile 语法通过**；`verified_commit` 待本地验证后回填（不伪造）。TASK 交付证据已回填，spec_sync=clean、规范影响 none。
- **TASK-M2-APPOINTMENTS**：**Open（2026-08-12 进行中）**——SSE 实时刷新：新增 `GET /slots/events`（`streamSlotEvents`，interviewer 会话、无 CSRF）+ 前端 `InterviewView` 的 `EventSource` 订阅；复用 `slot_snapshot` 读路径每连接 1s 轮询派生变更，ownership 脱敏（none/self/other），同账号≤2 连接（超额 429 RATE_LIMITED），15s 心跳；无 Redis 中间件、无新迁移、不单列 REVIEW 任务。实现已提交，待本机验证批处理（同 M1 sandbox 限制）。
- **TASK-M3-APPOINTMENTS**：**Open（2026-08-12 进行中）**——通知 Worker（Outbox 消费 + SMTP 发信）：新增 `apps/api/app/notifications/`（email.py 渲染+发送、worker.py 轮询 Outbox）+ 重写 `worker.py` 入口；复用 `notification_events`（无新迁移）、`FOR UPDATE SKIP LOCKED` 原子领取、status 状态机承载 at-least-once 重试；解密复用 `_decrypt_appointment`，收件人=预约归属人注册邮箱；SMTP 163 凭据仅运行时环境变量。**飞书通道与 `notification_deliveries` 尝试历史表延后**（无飞书凭据 / 降低未验证 schema 风险）。待本机验证批处理。
