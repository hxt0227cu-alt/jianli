...
- **TASK-M6-DB**：**Open（2026-08-13 用户已批准 schema；迁移已编写；待 WSL 真实 PG 验证；不执行生产迁移）**——AI 问答域 4 表迁移（`conversations` / `conversation_messages` / `knowledge_documents` / `knowledge_index_versions` + 5 枚举 + 索引/约束），**全部出自已批准领域模型 v1.1.5 §6.13/6.14**，可逆迁移 `0004_aiqa_schema`（alembic head）。关键决策：`active_index_version_id` 循环 FK 后置补建；活跃文档 `content_checksum` 部分唯一索引（删除后可重传）；`RecommendedQuestionCache`（§6.16，P1-2 优化）**用户确认不纳入本次**。本地门禁 ruff/mypy/alembic heads 全绿；迁移测试 `tests/migrations/test_aiqa_schema.py` 待 WSL 真实 PG（`jianli_tc_aiqa_001_db`）。M6 二轮（会话持久化）/三轮（知识库摄取）实现依赖本任务验证通过。审批材料见任务单 `tasks/TASK-M6-DB.md` §7。
