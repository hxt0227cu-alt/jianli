...
- **TASK-FE-AIQA-001**：**Open（2026-08-13 前端 AI 问答页已实现，commit `82d6c19`；待 WSL 手动验证）**——ChatPanel 真实化：`POST /answers:stream` SSE 流式回答（fetch+ReadableStream 解析，sse.md §3 帧）、推荐问题加载与点击即问、引用（citations）展示、grounded/offtopic 徽标、错误态、回答中禁用输入；resume 页 live（page_key=resume），projects 页 page_key=projects+project_key（CSS 隐藏面板），interview/mine 页保留静态降级版；vite proxy 补 /pages /answers /conversations /admin；ProjectView 受控提升。门禁：typecheck ✅ + vitest 1 passed ✅（旧断言全保留 + 6 新锚点）+ vite build ✅（沙箱 safe-delete 拦 dist 清空，dist-check 验证产物正常）。详见任务单 `tasks/TASK-FE-AIQA-001.md`。
