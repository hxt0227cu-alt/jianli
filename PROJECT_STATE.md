...
- **TASK-M6-AI-QA**：**Open（规划中，2026-08-13 建；DB 迁移待用户批准）**——AI 问答域（RAG + 人格层/数字分身）：已批准 OpenAPI v0.2 的 `getPageContent`/`listRecommendedQuestions`/`streamAnswer`/`listConversations`/`createConversation`/`listConversationMessages` + 知识库摄取三件套。首轮可实现无新表依赖子集（`getPageContent`+`listRecommendedQuestions`+匿名 `streamAnswer`，基于既有页内容 grounding、不持久化）；**会话持久化与知识库摄取依赖 4 张未迁移表（`conversations`/`conversation_messages`/`knowledge_documents`/`knowledge_index_versions`），须先走独立迁移任务 TASK-M6-DB（人工审批建表）**。详见任务单 `tasks/TASK-M6-AI-QA.md`。
