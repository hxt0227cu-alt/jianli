...
- **TASK-AGENT-TOOLS-002**：**Open（2026-08-15 建；实现已提交 `f0a1ec5`，待 WSL 验证）**——接通 service 两阶段 function calling：`stream_answer` 第一轮带 `tools`（`tool_choice=auto`）调真实模型，**模型自主决定是否调用 `search_knowledge` 并生成检索词 query**；service 执行现有混合检索（`_knowledge_candidates` + `retrieve` 兜底）→ `tool_calls_frame`（模型 query + hits）→ 第二轮带【已知资料】生成。安全不变量不放松：模型不调工具 → 系统兜底检索原问题（保 grounded/评测稳定）；工具无命中一律走 offtopic 拒答（无资料永不编造，REJECT 保持）。门禁：ruff ✅ / mypy ✅（45 files）/ DB-free 16 passed ✅（`test_aiqa.py` 12 + 新增 `test_agent_tools.py` 4：模型 query 驱动检索 / 系统兜底 / 无命中拒答）。契约 `sse.md` §3 文字同步（query 模型生成、hits 可空）。**待用户 WSL 验证**：评测重跑保持 8/8+10/10（真模型自主决策下）+ 浏览器新会话提问看绿色条 query 为模型检索词。详见任务单 `tasks/TASK-AGENT-TOOLS-001.md` / `tasks/TASK-AGENT-TOOLS-002.md`。
