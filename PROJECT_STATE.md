...
- **TASK-M4-APPOINTMENTS**：**Open（2026-08-12 启动）**——注册 / 邮箱验证 / 密码找回（auth 域补全）：新增 `POST /auth/register`、`/verify-email`、`/resend-verification`、`/request-password-reset`、`/reset-password` 五个 operation，实现 SRS §5.6 注册/验证/找回语义；令牌 `token_hash` 存储、一次性+过期、找回后作废全部会话、限频防枚举、邮件 best-effort（SMTP 未配置 sink）；复用 BCrypt（`passwords.py`）、Redis 限频、notifications/email.py。复用 migration 0001 的 `email_verification_tokens` / `password_reset_tokens`（**无新迁移**）。**规范影响：OpenAPI v0.2 需补 5 个 operationId、测试计划需补 TC-AUTH-REG/VERIFY/RESET → 实现后做一次 impact review 将 spec_sync 转 clean**。SMTP 163 凭据仅运行时环境变量。待实现 + 真实 PG 测试通过。
