...
- **TASK-KB-EVAL-002**：**Open（2026-08-15 已实现并验证，verified；待用户授权关闭）**——判别式评测集（EVAL-001 升级）：语料 10 篇 + LITERAL 8 / SEMANTIC 6 / EXTREME-SEMANTIC 6 / REJECT 10。**实测数字（哈希 vs BGE-M3 两遍对比）**：全链路（混合检索）LITERAL 8/8 持平、SEMANTIC avg-rank 2.2→1.8；**纯向量层（`test_pure_vector_ranking` 绕过 BM25）EXTREME avg-rank 1.8→1.3**（6 条中 4 条 rank=1，哈希仅 2 条）。**关键认知（实测）**：CJK 单字 BM25 + 小语料使混合链路 top-6 全召回、向量差异被兜底稀释（证明混合检索对 embedding 退化鲁棒）；语义向量价值须在纯向量层量化。曾踩坑：`_settings` 漏读 embedding env 致两遍同用哈希（`9376767` 修复）；`dimensions` 参数硅基流动 400（`0aac1f8`）。`verified_commit=118a02d`。详见任务单 `tasks/TASK-KB-EVAL-002.md`。
- **TASK-KB-EMB-001**：**Open（2026-08-14 已实现并验证，verified；待用户授权关闭）**——真实 embedding 接入（硅基流动 BGE-M3，1024 维）：迁移 0007 `knowledge_chunks.embedding` 768→1024（DROP+ADD，旧向量丢失需重灌）+ config 默认维度 768→1024 + `_vector_dimension` 维度断言。用户选定 BGE-M3 即预批准 0007。**用户 WSL 验证**：seed 5 chunks 真实 BGE-M3 + 评测 HIT=8/8 保持；`verified_commit=d0a6cb8`（含 `0aac1f8` dimensions 修复）。详见任务单 `tasks/TASK-KB-EMB-001.md`。
