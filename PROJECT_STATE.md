...
- **TASK-M6-DB**：**Open（2026-08-13 建，待用户审批 schema 并授权编写迁移；不执行生产迁移）**——AI 问答域 4 表迁移（`conversations` / `conversation_messages` / `knowledge_documents` / `knowledge_index_versions` + 5 枚举 + 索引/约束），**全部出自已批准领域模型 v1.1.5 §6.13/6.14**，可逆迁移 0004。关键决策：`knowledge_documents.active_index_version_id` 循环 FK 后置补建；活跃文档 `content_checksum` 部分唯一索引（删除后可重传）实现"相同文件去重"；`RecommendedQuestionCache`（§6.16，P1-2 优化）**不纳入本次**。M6 二轮（会话持久化）/三轮（知识库摄取）实现依赖本任务批准。审批材料见任务单 `tasks/TASK-M6-DB.md` §7（schema 全文）。
