...
- **TASK-DB-002 / TASK-REVIEW-DB-002**：**In Progress / Open（2026-08-11）**——用户已批准 `437f45c` 预约域五张核心表迁移评审包；正在实施可逆 migration 与真实 PostgreSQL 验证，未执行生产迁移、未进入 BOOKING-001。
- **TASK-TEST-MIGRATION-001**：**In Progress（2026-08-11）**——用户已批准修正 DB-001 冻结测试对后续合法表的兼容性；仅调整表集合断言，身份域精确 schema/约束断言保持冻结。
- **TASK-TEST-DB-002-CONSTRAINTS**：**In Progress（2026-08-11）**——DB-002 测试因格式化后超过 320 行预算而按规则拆分；独立承载 approved 约束拒绝路径，不修改 migration 或冻结断言。
- **TASK-TEST-DB-002-REFERENTIAL**：**In Progress（2026-08-11）**——约束测试完整覆盖经格式化后仍超过 190 行预算，再拆出三个 enum 与普通 FK 拒绝路径；不修改 migration 或冻结断言。
