...
这个项目不是"FastAPI + 向量库 + 前端"的套壳 demo。以下每一项都是真实生产约束下踩出来的硬问题，仓库内有对应代码、测试与 ADR：

| 难点 | 常见做法（做不到） | 本项目做法 |
|------|------------------|-----------|
| **同一时段被两人同时约走** | 查一下库存就 INSERT，并发下必超卖 | 时段物化 + **统一锁顺序 L0→L3** + 行锁 `FOR UPDATE` + 客户端 `Idempotency-Key` + 乐观版本号，预约/改期/取消同事务，高并发用例真实覆盖 |
| **RAG 一本正经地胡说** | 检索到啥喂啥，模型自己编 | **证据门**：最小相似度阈值 + 引用必须来自命中片段；无依据直接 `offtopic=true` 拒答，**不允许模型自由发挥** |
| **Agent 调工具越权/乱约** | 前端把工具结果直接显示 | 模型只输出工具意图，**服务端不信任模型**：白名单内工具经 RBAC（面试官仅本人、owner_admin 旁路）+ 复用同一套 `BookingService` 事务二次校验，最多 4 步、防注入 |
| **邮件/飞书发了但业务回滚** | 业务提交后直接调外部 API，失败就丢 | **Outbox 模式**：业务事务同库写 `NotificationEvent`，独立 Worker 领取/重试/超时回收，与外部发信解耦 |
| **简历/手机号落库被拖库** | 明文存数据库 | 敏感字段 **AES-256-GCM** 字段级加密；公司名只存 HMAC 指纹；日志/响应/Trace 全链路扫 PII |
| **AI 改代码把需求改飞** | 让 AI 自由发挥 | **任务单驱动 + 规范唯一真相源（SSOT）+ 双角色审查**：每个变更有 TASK 约束范围，规格/代码/运行三方事实源分离，CI 门禁卡合并 |

> 这些不是"用了什么技术栈"，而是"在并发、可信度、安全、治理四个维度上，为什么不能照抄教程"。完整论证见 [docs/design/security.md](docs/design/security.md)、[docs/design/domain-model.md](docs/design/domain-model.md) 与 `docs/adr/`。
