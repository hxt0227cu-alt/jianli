...
项目获国家级大创立项；对应挑战杯科技发明制作 A 类赛事路演资格。识别准确率 ≥95% 是申报书目标指标，不表述为已经实测达到。""",
    ...
    "jianli-evaluation-ci.md": """# Jianli 评测中心与 CI 门禁\n评测中心读取版本化 JSON 报告，公开样本数、生成时间、verified commit、套件结果与脱敏边界案例，而不是运行时临时拼一个满分。当前报告为 79/79：Agent/Trace 22、RAG 事实一致性 38、Web 交付 1、Cross-Encoder 协议 4、语义缓存与 Provider 韧性 8、多副本共享熔断 6；真实 RAG 测试会上传 canonical corpus、分块、BGE-M3 embedding 入 pgvector，再验证命中、拒答、隐私和误拒；版本化报告含越界用例结果 10/10 拒答（拒答率 100%，相对早期 0% 提升），在真实 embedding + 0.47 阈值下验收。Agent Quality Gate 定义 backend-agent→rag-integration→web-delivery 三个串行 job，后两段分别带真实 PG/Redis 和前端测试/typecheck/build。目前仅有本地等价门禁（单测、类型检查、构建）通过的证据，GitHub Actions 尚无远端 run；79/79 只说明这组冻结检查在本地全过，不代表生产准确率或线上可用性，不能据此宣称云端流水线已跑绿。""",
