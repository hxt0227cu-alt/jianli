...
CORPUS: dict[str, str] = {
    ...
    "litchi.md": (
        ...
        "Spring Boot（自定义 Bearer 会话 Token，AuthInterceptor + @AuthRequired + "
        "@RoleAllowed 鉴权）；业务层 Spring Boot 3.2（智能问答/病害识别/门店协同/知识文档/"
        "评测中心/用户反馈/系统状态监测等）；"
        ...
        "（准确性/完整性/可执行性/安全性 5/3/1 分制）；"
        "满意度反馈（总体评分/准确性/实用性/流畅性）。\n"
        "## 系统测试（论文 TC）与验证报告\n"
