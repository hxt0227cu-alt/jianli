...
CORPUS: dict[str, str] = {
    ...
    "litchi.md": (
        ...
        "## 规模与实现\n"
        "一人独立完成，8 个模块：backend（Spring Boot 3.2 / Java 17，100 文件 / "
        "12,622 行）、frontend（Vue3 + TS，34 文件 / 11,076 行，22 个业务页面按"
        "农户/门店/技术员三角色组织）、diagnosis-service（Python YOLOv8，592 行）、"
        "data-platform、observability、deploy/helm、benchmarks、datasets。"
        "MySQL 14 张表（platform_* 系列），Controller 接口 49 个。\n"
        "## 受控 Agent 实现细节\n"
        "全部实现在 AgentService.java（458 行）：Planner 用 LLM 生成 JSON 计划"
        "（硬上限 4 步，解析失败走确定性 fallbackPlan）→ Executor 顺序执行 → "
        "Synthesizer 只依据工具证据作答、模型不可用返回降级文案。Guard 无独立类、"
        "职责内嵌：计划阶段拦未知/重复工具与超预算截断（availableTools.containsKey "
        "+ maxSteps）；RBAC 靠 AgentTool.supports(user) 按角色过滤工具；HITL 靠 "
        "requiresApproval() 把唯一写工具 pending_remedy_plan 置 waiting_approval，"
        "confirm(approve/reject) 后才落库。\n"
        "## 检索与图谱\n"
        "Milvus collection litchi_knowledge、1024 维（SimpleEmbeddingService 哈希向量"
        "——在无 GPU 笔记本可本地演示的约束下选择，CPU 可跑）、COSINE、AUTOINDEX；"
        "Neo4j 管品种/病害/虫害/药剂/栽培技术实体及 HAS_DISEASE/TREATS/PREVENTS/"
        "NEEDS_TECHNIQUE 关系（不可用时回退内置图谱）。融合方式：ChatService 同时调"
        "向量召回（top4）与图谱查询合并进 prompt；Agent 侧拆 knowledge_search / "
        "knowledge_graph 两个独立工具取证据，Synthesizer 统一综合。\n"
        "## 模型与降级\n"
        "LLM 用本地 Ollama 的 qwen2.5:0.5b（无 GPU 笔记本 CPU 可跑，不依赖外部 API）；"
        "诊断服务 YOLOv8 病害分类（实际训练 5 类，normalize_label 映射 7 类中文声明，"
        "从零训练 top1 43.75% → 微调 93.75%、最终权重 91.25%、top5 全程 100%），"
        "推理失败按 yolo→dataset-vision→demo-rule 三级降级；病害识别准确率由 20% 提升至 93.75%"
        "（top1 43.75% → 微调 93.75%），Chat P95 由 5s 降至 124ms（约 1/50），"
        "50 并发成功率 100%。\n"
        "## 可观测性\n"
        ...
        "监测建议、广东省/海南省农业农村厅方案月历、深圳地方标准、华南农大研究等 19 篇"
        "权威资料 + 30 个 raw 文件（22 PDF 学术论文 + 7 HTML）清洗后 29 篇 md。\n"
        "## 已知局限（诚实记录）\n"
        "并发压测 200 并发仅 19% 成功、P95 15.18s，修复旧接口路径/PowerShell 5 兼容/"
        "外部依赖重复探测/文档检索对象级同步（CopyOnWriteArrayList）后仍不达标，"
        "按时间边界停止调优，原始报告如实保留（README 与 KNOWN_LIMITATIONS 均如实"
        "列为未通过）。Agent 运行用单表 JSON 快照 + 内存回退（未拆 step/审批独立表、"
        "无法跨实例恢复）；混合检索（BM25+RRF）与模型 rerank 为未完成项；数据平台"
        "（Kafka CDC/ClickHouse/Helm）为可部署模板、未生产验证。\n"
        "## 毕设论文版（论文真源，2026 届优秀毕设 90.4 分）\n"
        "论文题目《基于大模型 RAG 的荔枝智能问答平台设计与实现》（英文：Litchi Intelligent "
        "Question Answering and Collaborative Diagnosis Platform Based on Large Model RAG），"
        "[学校已脱敏]人工智能学院，指导教师王潇、胡鑫（校外），答辩 2026-05-10。\n"
        "系统定位：集荔枝病虫害知识管理、智能问答、图像辅助诊断、防治方案推荐和门店协同服务"
        "于一体的智能诊断平台，面向农户、门店、系统管理员三类角色。\n"
        "## 五层架构（论文版）\n"
        "表现层 Vue3 + TypeScript + Element Plus（Three.js 程序化荔枝 3D 模型）；接入层 Nginx + "
        "Spring Boot（自定义 Bearer 会话 Token，AuthInterceptor + @AuthRequired + @RoleAllowed 鉴权）；"
        "业务层 Spring Boot 3.2（智能问答/病害识别/门店协同/知识文档/评测中心/用户反馈/系统状态监测等）；"
        "AI 服务层 SimpleEmbeddingService（1024 维哈希向量）+ VectorSearchService（Milvus COSINE + "
        "AUTOINDEX）+ KnowledgeGraphService + LLMService（Ollama Qwen2.5:0.5b）+ DiagnosisService"
        "（YOLO 推理/特征匹配/规则兜底）；数据层 MySQL 8.0 + Neo4j 5.15 + Milvus 2.3.3 + MinIO"
        "（会话存 MySQL 并保留本地 JSON 回退）。部署：Docker Compose 10 服务（mysql/neo4j/etcd/"
        "minio/milvus/ollama/ollama-model-init/diagnosis-service/backend/frontend）。\n"
        "## RAG 问答链路（论文版，非四段 Agent 表述）\n"
        "查询（ChatService 校验+向量/图谱开关+领域关键词与图谱术语抽取）→ 检索（Milvus 向量默认 "
        "4 条 + Neo4j 图谱并行混合）→ 候选片段筛选（DocumentService：Milvus 相似度 + 本地 chunk "
        "相似度 + 词面增强分数去重排序补充）→ 生成（Qwen2.5:0.5b：llmTimeoutMs 45000、temperature "
        "0.2、numPredict 220、keepAlive 30m；systemPrompt 严格基于参考资料、资料不足直接说明"
        "「不足以判断」不编造、3 句话内并给一条操作建议）→ 证据约束（prompt 带编号文档片段 + 图谱"
        "实体，回答同步展示来源与图谱命中）→ 降级（buildFallbackAnswer 依托已检索证据生成结构化"
        "兜底，不掩盖服务异常状态）。分块策略：CHUNK_SIZE=480、CHUNK_OVERLAP=120（25% 重叠）。\n"
        "## 病害识别（论文版）\n"
        "YOLOv8（yolov8-litchi.pt，Ultralytics，服务启动预加载）+ 标签映射（healthy→健康叶片/"
        "anthracnose、black spot→炭疽病/leaf blight、downy、mildew→霜疫霉病/red rust、rust→红锈病/"
        "worm、borer、insect→虫害；无检测结果返回「健康叶片」置信度 0.88）+ 三级降级（L1 yolo 正常 "
        "推理 → L2 dataset-vision 数据集特征匹配：每类 8 张、48x48 缩略图颜色均值 + 16 格灰度直方图、"
        "score = 1/(1+distance) 取 Top3 → L3 demo-rule 规则兜底：filename_hint 0.74 / SHA256 取模 "
        "3 确定性）+ MODEL_LOCK（threading.Lock 单推理互斥）并发安全。\n"
        "## 数据库与服务类（论文版）\n"
        "核心服务类 9 个：AuthService / ChatService / DocumentService（支持 txt、md、csv、json、pdf、"
        "docx）/ VectorSearchService / KnowledgeGraphService / LLMService / CollaborationService / "
        "EvaluationService / MysqlStateStoreService（集中维护 10 张 MySQL 业务表 + 本地 JSON 回退）。"
        "表：platform_users、platform_sessions、platform_chat_messages（含 sources_json 与 "
        "knowledge_graph_json 问答溯源）、platform_documents（chunk_count / is_indexed / storage_path）、"
        "platform_document_chunks（vector_json）、platform_store_profiles、platform_remedy_plans"
        "（products_json / usage_tips_json / risk_notes_json）等。\n"
        "## 评测体系（论文版）\n"
        "评测中心模块：题库按荔枝品种/栽培技术/病害识别/虫害防治/农药使用/综合问题分类；自动评分"
        "（参考答案 + 关键词匹配 + BLEU 分数，维度=准确性/安全性/完整性/可操作性）；人工复核"
        "（准确性/完整性/可执行性/安全性 5/3/1 分制）；满意度反馈（总体评分/准确性/实用性/流畅性）。\n"
        "## 系统测试（论文 TC）与验证报告\n"
        "健康检查：Neo4j/Milvus/Ollama/diagnosis 均 connected、识别引擎 ultralytics-yolo、文档索引 "
        "55/55；炭疽病识别置信度 0.4984（非演示模式）；智能问答返回来源数量 4；防治方案推荐 3 个"
        "（选中 plan-demo-anthracnose，推荐门店「荔园农资服务站」）；求助工单创建/状态流转正常；"
        "病害趋势：炭疽病累计咨询 12 次、近期 4 次。仓库验证报告：主流程彩排 + 30 分钟稳定性 119 轮"
        "全部成功 + 50 并发问答请求全部成功。\n"
        "## 四大亮点（论文）\n"
        "① RAG + 知识图谱双增强架构（结构化关系 + 非结构化文本双约束，量化提升待专项对比实验验证，"
        "不做无数据支撑的效果预判）；② 三级降级保障机制（模型不可用仍输出可解释诊断结果）；③ 多角色"
        "业务闭环（农户/门店/系统管理员三位一体，线上咨询→线下处置→后台维护打通）；④ 可进化评测"
        "体系（评测记录 + 满意度反馈可追溯）。\n"
        "## 存在不足（论文自述，诚实记录）\n"
        "知识覆盖以荔枝病虫害通识为主，特定品种与地域气候精细管理知识待补；病害识别在复杂光照与"
        "早期症状场景的稳定性待更多田间数据验证；轻量化模型复杂推理专业深度有限；单机部署并发承载"
        "有限；尚未经过大规模果园实地验证。"
    ),
    "taiyizhi.md": (
        ...
        "sleep_improvement 5、voice_companion 5。曾因评测继承 AGENT_MODEL_PROVIDER=openai_compatible "
        ...
        "MainActivity 一行 extends BridgeActivity，零自定义原生代码）。固件 13,223 行 C++ / 20 文件："
    ...
    "interview-story.md": (
        ...
        "的一部分，让任何接手的人（同事或 AI）都能无缝上手，我带来的不是一次性代码而是可持续的工程。\n"
        "## 求职动机\n"
        ...
        "输出重点巡检区域。成果：完成实物中试/原型，已落地实测（识别准确率 ≥95% 为申报书目标指标）；"
