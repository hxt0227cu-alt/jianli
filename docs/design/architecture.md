...
> **文档状态**：v0.5 · `status = approved`（用户于 2026-08-27 批准 ADR-RESILIENCE-001：Semantic Cache 与 Provider 熔断）。
...
### 9.4 故障降级（SRS §5.4）
- LLM 与 Reranker provider 使用独立 Circuit Breaker：连续失败打开、恢复窗口后单探针 half-open、成功关闭；open 时直接走既有 `MODEL_UNAVAILABLE` 或 RRF 回退，不切换第二模型（ADR-RESILIENCE-001）。
