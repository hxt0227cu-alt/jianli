...
- **事务**：给出唯一全局锁顺序（Company → Appointment → CompanyBookingException → AppointmentSlot 按 start_at 升序）；四条流程均"先加锁后校验"，`FOR UPDATE` 不带 status 过滤；创建校验数量/同日/连续性/available；例外在同一事务 `FOR UPDATE` 校验并消费并写 `dedupe_exception_id`；改期先锁 Appointment 校验 active/归属/version，再合并新旧 Slot 统一升序加锁，占新、释旧（按 §4.6 重新物化）、更新 Appointment、撤销旧提醒、写新 Outbox 同事务；用户取消释放格按 §4.6 重新物化（不再无条件 available）；owner 强制取消写 `owner_locked`；两流程均写通知事件（owner 另写 AuditLog）；`AvailabilityOverride` 创建/修改/删除事务（§4.7 重写为 8 步：**① §4.0 新增锁层级 `L2.5 AvailabilityOverride`（按 id，强制先于 L3）——UPDATE/DELETE 须 `SELECT ... FOR UPDATE` 锁自身行取【当前真实】`old_range`，命中 0 行即 ROLLBACK，禁用前端传入旧值**；② 据真实值算 `affected_range = old_range ∪ new_range`（不相交按两段一次性升序加锁）；**③ CREATE/UPDATE 范围须对齐并命中 ≥1 个现存 `AppointmentSlot`，命中 0 → ROLLBACK 拒绝（不得创建无锁定载体的 override；DELETE 豁免）**；④ 统一锁范围内**全部** Slot 含 `booked`；⑤ 锁后复检冲突排除自身 id；⑥ 无冲突才写；⑦ 仅对 `appointment_id IS NULL` 的 Slot 重新物化；⑧ 提交）；§4.5 并发矩阵补 3 行（同一 override 并发 UPDATE/DELETE、override vs 改期/取消、范围不命中 Slot）。
- **Outbox**：事件级原子领取（`FOR UPDATE SKIP LOCKED` + `uq_delivery_attempt` 二重互斥，仅防重复建行）；**新增投递级原子领取**（§6.3.2：`NotificationDelivery` `queued→sending` 短事务 `FOR UPDATE SKIP LOCKED` + RETURNING，提交后才调外部，`uq_delivery_attempt` 不防同一行重复发送）；**`created_at` 仅表投递行创建时间（非领取时刻）**，`Txn D` 仅领剩余租约足以覆盖「外呼超时+余量」的 `queued` 行（临近/超租约留 Sweeper 按未发送回收）；隐式租约（非终态 + `created_at`，5min）超时回收，且**区分 `queued`（未发送，`queued_lease_expired`）/ `sending`（结果未知，`sending_lease_expired_unknown`）两类超时与不同 `last_error`**，外部调用超时须远小于 5min；**`Txn W` 回执 CAS 写回**（§6.4.1：`WHERE id=:id AND status='sending'`，命中 0 行=已被 Sweeper 回收、迟到 Worker 不覆盖 `retry_scheduled`/`dead_letter`、仅记告警）；**`Txn W` SQL 须逐字段对齐领域模型 v1.1.5 §6.12**：不得引用不存在的 `NotificationDelivery.version`（幻列，本表无乐观锁列，CAS 由 `status='sending'` 承担）、`provider_message_id` **写独立列**（不得塞进 `channel_metadata`，否则 §7 按该列反查失效）、`channel_metadata` 用 `COALESCE(channel_metadata,'{}'::jsonb) || :meta` **JSONB 合并不得整体覆盖**（否则抹掉 §7 已写退信键）、`:meta` 按通道白名单（email→`smtp_accepted_at`；feishu→`provider_request_id`/`feishu_record_id`/`response_code`）、**`bounced_at`/`bounce_reason` 仍只由 §7 退信处理回写，Txn W 禁写**；正文须含全 12 列逐字段核对表（标明 Txn W 只写 4 列，`event_version`/`attempt_no`/`next_retry_at`/`created_at` 不由回执写）；明确至少一次语义；稳定幂等键**不含 attempt_no**、手动重发经 `event_version` 有意区分；`NotificationEvent`/`NotificationDelivery` 状态转换逐条对齐 SRS §6.2 与领域模型 §5，不新增状态。
...
- Schema/并发收口（2026-08-09 续二，两项）：3a18b7f（docs/design/architecture.md 单文件；① §6.4.1 `Txn W` SQL 删除幻列 `NotificationDelivery.version`、`provider_message_id` 写独立列、`channel_metadata` 改 `COALESCE(...,'{}'::jsonb) || :meta` JSONB 合并、新增全 12 列逐字段核对表对齐领域模型 v1.1.5 §6.12、`:meta` 通道白名单、bounce 键仍只由 §7 回写；② §4.0 新增 `L2.5 AvailabilityOverride` 锁层级 + 强制规则 5，§4.7 重写为 8 步（先 `SELECT ... FOR UPDATE` 锁自身行取真实 `old_range`、CREATE/UPDATE 范围须命中现存 Slot 否则 ROLLBACK、再锁全部 Slot 含 booked、锁后复检排除自身 id、无冲突才写、仅物化 `appointment_id IS NULL`）、§4.5 矩阵增 3 行、§11.2 登记对外错误码命名开放项；仍 review 不批准）
- 修改文件清单（G1 初版）：docs/design/architecture.md / tasks/TASK-ARCH-002.md / tasks/TASK-ARCH-001.md / docs/baseline.yml / PROJECT_STATE.md（5 个路径，与「允许修改路径」逐一对照一致）；本轮补充修正（1e0d9ed）仅 docs/design/architecture.md 1 个路径；并发竞态修正（12dcd2d）仅 docs/design/architecture.md 1 个路径；Schema/并发收口（3a18b7f）仅 docs/design/architecture.md 1 个路径。以上三轮修正的 TASK/PROJECT_STATE 回填分别落在 cc9d8fa / 981a044 / 本轮同步提交，均在允许修改路径内。
- 测试命令及结果：
...
- 7. Schema/并发收口一致性（3a18b7f）：`grep -n "version = version + 1" architecture.md` → 仅剩 §6.4.1 核对表与 §12.3 条目 22 的**说明性引用**，SQL 语句体内已无（pass）；`grep -n "provider_message_id = :provider_message_id" architecture.md` → 出现于 §6.4.1 Txn W（pass）；`grep -n "COALESCE(channel_metadata" architecture.md` → 出现（JSONB 合并，pass）；`grep -c "L2.5" architecture.md` → 11 处（§4.0 锁表 + 规则 5 + §4.7 步骤 1 + §4.5 矩阵，pass）；`grep -n "WHERE id = :self_id" architecture.md` → 出现于 §4.7 步骤 1 `FOR UPDATE`（pass）；`grep -c "OVERRIDE_RANGE_EMPTY" architecture.md` → 4 处（§4.5 / §4.7 / §11.2 / §12.3，pass）；逐字段比对领域模型 v1.1.5 §6.12 全 12 列（id/event_id/delivery_purpose/channel/event_version/attempt_no/status/channel_metadata/provider_message_id/next_retry_at/last_error/created_at）→ 架构 SQL 不再引用不存在的列（pass）。
...
- 未解决风险：
  ...
  2. **开放项待用户裁定**：ADR-005/006/007/008 留《安全设计》；`AUTH_EXPIRED` 语义冲突（SRS §3.3 关联限频 vs §8 定义登录过期）仍留作 OpenAPI 前阻塞项。**§11.2 取消释放目标状态已在本轮裁定并并入 §4.6（按 AvailabilityOverride 与日历规则重新物化），§13 待办已删除，正文与验收一致。** **新增开放项（§11.2）**：§4.7 两个拒绝语义的**对外错误码命名**（`OVERRIDE_NOT_FOUND` / `OVERRIDE_RANGE_EMPTY`）在架构文中为**内部占位名，非已批准 SRS §8 既有错误码**；架构层只约束「两种情形必须 ROLLBACK 拒绝」的行为，码值与文案留《接口契约（OpenAPI）》裁定，若需新增 SRS 错误码须走 Change Request——本轮**未**在架构中主张 SRS 已定义该二者，不构成规范扩展。
  4. **§4.7 并发与范围校验（转《测试计划》）**：须覆盖「两事务并发 UPDATE 同一 override 至不同新范围」→ 断言 L2.5 行锁串行化、后到者基于重读的真实 `old_range` 重算 `affected_range`；「并发 UPDATE vs DELETE 同一 override」→ 断言后到者 0 行回滚；「创建范围不命中任何 Slot 的 override」→ 断言拒绝；「override 变更 vs 改期/取消同一时段」→ 断言由 Slot 行锁串行化且 `booked` 格保持 `booked`。
...
- spec_sync：clean（上游 SRS v1.1 / 领域模型 **v1.1.5** / UI 线框 v1.0 均 approved 且 based_on 未变）
  > 记账更正：本行原记「领域模型 v1.1.4」，与 architecture.md 头部 `based_on` 及 `docs/baseline.yml` 实际值 **v1.1.5** 不符，属本任务自身记录的版本号笔误，本轮如实更正（不改变 spec_sync 结论，不涉及上游工件内容）。
- verified_commit：3a18b7f（2026-08-09 续二，两项 Schema/并发收口，最新 review 草案快照；含 1e0d9ed 三项实现正确性 + 12dcd2d 两项并发竞态修正；仍 review 不批准，非自指）
