...
- **事务**：给出唯一全局锁顺序（Company → Appointment → CompanyBookingException → AppointmentSlot 按 start_at 升序）；四条流程均"先加锁后校验"，`FOR UPDATE` 不带 status 过滤；创建校验数量/同日/连续性/available；例外在同一事务 `FOR UPDATE` 校验并消费并写 `dedupe_exception_id`；改期先锁 Appointment 校验 active/归属/version，再合并新旧 Slot 统一升序加锁，占新、释旧、更新 Appointment、撤销旧提醒、写新 Outbox 同事务；用户取消与 owner 强制取消分别释放为 `available` / `owner_locked` 并写通知事件（owner 另写 AuditLog）。
- **Outbox**：`FOR UPDATE SKIP LOCKED` 原子领取 + `uq_delivery_attempt` 二重互斥；隐式租约（非终态 + `created_at`）超时回收；明确至少一次语义；稳定幂等键**不含 attempt_no**、手动重发经 `event_version` 有意区分；`NotificationEvent`/`NotificationDelivery` 状态转换逐条对齐 SRS §6.2 与领域模型 §5，不新增状态。
