...
## 5. 验收（2026-08-14 已完成）
- [x] ruff ✅ + mypy ✅（沙箱）
- [x] WSL 真实 PG：`PYTHONPATH=. pytest tests/aiqa/test_rag_eval.py -v` → **1 passed + 1 xfailed in 6.73s**
- [x] 命中数字：**HIT = 8/8 (100%)**（逐条 grounded=True + 期望 doc 在 citations）
- [x] 拒答数字：**REJECT = 0/6 (0%)**，全部 XFAIL——6 条均 `offtopic=False grounded=True`，P1 无相关性阈值缺陷的可测量证据（预期行为，非回归）
- [x] 任务单交付证据回填（`verified_commit=b20f67d`，任务待用户授权关闭）

## 6. 面试价值（写进简历/话术）
"RAG 评测回归：14 条评测集（命中/越界/无依据），**命中率 100% / 拒答率 0%（已知缺陷：检索无相关性阈值，已量化待修）**，一条命令可跑；换 embedding 后数字对比（哈希 → 真实）证明检索质量可量化、可优化。"

## 7. 诚实标注（限制）
- 当前语料 4 篇（每篇 ~1 chunk），top-6 候选基本覆盖全部文档 → "期望 doc 在 citations" 断言偏宽松；命中 100% 含候选覆盖效应。扩语料（≥10 篇/更细 chunk）后区分度增强，是后续优化项（P1 一起做）。
- 拒答 0/6 是**当前缺陷的真实基线**，不是失败回归——正是评测集的价值：把"靠肉眼发现不了的问题"变成数字。
