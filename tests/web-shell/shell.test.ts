...
describe('web shell acceptance surface', () => {
  it('keeps the three static destinations and evidence boundaries', () => {
    ...
    expect(source).toContain('92.0% · 4.1%');
    expect(source).toContain('5 AGENTS · P-E-V');
    expect(source).toContain('P-G-E-S · 7 状态');
    ...
    expect(source).toContain('设备数据到分析结果，链路每一跳都有重试与幂等');
    expect(source).toContain('运行时、治理与评测，按证据口径拆开说');
    expect(source).toContain('一个人交付，不等于把所有责任都交给模型');
    expect(source).toContain('完整业务闭环之外，更值钱的是我知道数据能证明什么');
    expect(source).toContain('80+ · &gt;99%');
    expect(source).toContain('50C OK · 100C FAIL');
    expect(source).toContain('可追问但不公开原始证据');
    expect(source).toContain('159.88MS');
